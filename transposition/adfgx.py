...
class ADFGX:
    def __init__(self, key: str, alphabet: str = 'ABCDEFGHIKLMNOPQRSTUVWXYZ') -> None:
        ...
        self._map = {c: tuple(map('ADFGX'.__getitem__, divmod(i, 5))) for i, c in enumerate(self._alpha)}
    ...
    def __str__(self) -> str:
        return '\n'.join(map('  '.join, zipper(self._alpha, 5)))
...
def test_adfgvx():
    cipher = ADFGX('key', 'AZERTYUIOPQSDFGHJKLMWXCVB')
    print(cipher.encode('DCODE'), cipher.decode('FDFXFFFFXGAX'))
    cipher = ADFGX('queenly', 'AZERTYUIOPQSDFGHJKLMWXCVB')
    print(cipher.encode('attack at daw'), cipher.decode('AXAXAGFXXFXAAAAAAAAAXFFX'))
